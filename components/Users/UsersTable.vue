        ...
        <UTable v-model:sorting="sorting" :data="users" :columns="columns" @select="onSelect">
            ...
            <template #roles-cell="{ row }">
                <div class="flex flex-wrap gap-1">
                    <UBadge v-for="role in row.original.roles" :key="role" :label="role" :variant="getRoleVariant(role)"
                        :color="getRoleColor(role)" size="sm" />
                </div>
            </template>

            <template v-if="editable" #actions-cell="{ row }">
                <div class="flex justify-end">
                    <UButton icon="fluent:edit-16-regular" size="sm" color="neutral" variant="ghost"
                        aria-label="Voir l'Employé" @click="$emit('edit', row.original)">
                        Modifier
                    </UButton>
                </div>
            </template>
